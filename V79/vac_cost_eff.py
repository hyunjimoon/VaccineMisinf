...
def tf7_f3x_p(vv):
    ...
    x_marg_sorted = []
    x_label_sorted = []
    for marg, avg, label in sorted(zip(x_marg, x_avg, x_label)):
...
def tf8_x_ch2(vv):
    ...
    matplotlib.pyplot.rcdefaults()
    # ADDITIVE CENTERING is done when stored
    death_vals = vv['f1x_ch2'].sel(costcomp_tv='death', f1x='mvv') / 10 ** 12
    gdp_vals = vv['f1x_ch2'].sel(costcomp_tv='gdp', f1x='mvv') / 10 ** 12
    hosp_vals = vv['f1x_ch2'].sel(costcomp_tv='hospitalization', f1x='mvv') / 10 ** 12

    plot_d = matplotlib.pyplot.bar(vv.coords["h2"].values, death_vals, width=.5)
    plot_d.set_label("Deaths")
    plot_g = matplotlib.pyplot.bar(vv.coords["h2"].values, gdp_vals, bottom=death_vals, width=.5)
    plot_g.set_label("GDP")
    plot_h = matplotlib.pyplot.bar(vv.coords["h2"].values, hosp_vals, bottom=death_vals + gdp_vals, width=.5)
...
def tf9_f2x_dt(vv):
    ...
    for m_or_a, i in zip(vv.coords['f4x'].values, [0, 1]):
        ...
        axes[i].bar(vv.coords["time"].values, death_vals, width=.5, label = "Death")
        axes[i].bar(vv.coords["time"].values, gdp_vals, bottom=death_vals, width=.5, label = "GDP", )
        axes[i].bar(vv.coords["time"].values, hosp_vals, bottom=death_vals + gdp_vals, width=.5, label = "Hospitalization", )
        plt.legend()
        axes[i].set_xlabel("time")
        axes[i].set_ylabel(f"{m_or_a} vaccine value per person")
    plt.show()
    plt.savefig(f"../plot/tf9_f2x_dt.png")
    return
